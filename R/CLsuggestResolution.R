#' Suggest Optimal Resolution for Cell Ontology Term Roll-up
#'
#' @description
#' Sweeps a range of \code{min_group_size} values (and optionally
#' \code{max_ancestor_count} values) for \code{\link{CLrollup}} and recommends
#' the parameter combination whose resulting number of groups is closest to
#' \code{target_groups}.
#'
#' @section Depth convention:
#' The \code{max_ancestor_count} parameter passed to \code{\link{CLrollup}}
#' uses the package-wide ancestor_count definition (see \code{\link{CLdepth}}).
#'
#' @param ids Character vector of CL IDs.  Duplicates are silently removed.
#' @param clData An \code{ontology_index} object returned by \code{CLload()}.
#' @param target_groups Target number of final groups (default: \code{10}).
#' @param max_ancestor_count_values Integer vector of \code{max_ancestor_count}
#'   values to sweep in addition to \code{NULL} (no limit).  Default:
#'   \code{NULL} (sweep only \code{min_group_size}, no ancestor_count filter).
#'   Pass e.g. \code{c(5L, 10L, 20L)} to also test ancestor_count caps.
#' @param verbose Logical; if \code{TRUE} (default), print a summary.
#'
#' @return Invisible list with:
#' \describe{
#'   \item{\code{best}}{Named list with the best parameter configuration.}
#'   \item{\code{all_candidates}}{List of all tested configurations.}
#' }
#'
#' @export
#'
#' @examples
#' \dontrun{
#' clData <- CLload()
#'
#' ids <- c(
#'   "CL:0000624", "CL:0000625", "CL:0000236", "CL:0000576",
#'   "CL:0000623", "CL:0000775", "CL:0000771", "CL:0000767",
#'   "CL:0000451", "CL:0000097", "CL:0000235"
#' )
#'
#' suggestion <- CLsuggestResolution(ids, clData, target_groups = 3)
#'
#' # Apply recommended parameters
#' result <- CLrollup(ids, clData,
#'                    min_group_size     = suggestion$best$min_group_size,
#'                    max_ancestor_count = suggestion$best$max_ancestor_count)
#' }
CLsuggestResolution <- function(ids,
                                clData,
                                target_groups            = 10L,
                                max_ancestor_count_values = NULL,
                                verbose                  = TRUE) {

  # ---- Validate inputs ----
  .validate_cldata(clData)

  ids <- .validate_ids(ids, clData = NULL, unique_only = TRUE,
                       allow_unknown = TRUE, warn_invalid = TRUE)

  if (!is.numeric(target_groups) || length(target_groups) != 1L ||
      is.na(target_groups) || target_groups < 1) {
    stop("`target_groups` must be a positive integer.", call. = FALSE)
  }
  target_groups <- as.integer(target_groups)

  if (target_groups > length(ids)) {
    warning("`target_groups` (", target_groups,
            ") is larger than the number of input IDs (", length(ids), ").",
            call. = FALSE)
  }

  # ---- Build sweep grid ----
  max_min_size <- max(2L, as.integer(floor(length(ids) / max(1L, target_groups))))
  min_sizes    <- seq.int(from = 2L, to = max_min_size, by = 1L)

  # Combine max_ancestor_count values: always include NULL (no limit)
  mac_values <- c(list(NULL), as.list(as.integer(max_ancestor_count_values)))

  if (verbose) {
    message("Sweeping min_group_size from 2 to ", max_min_size,
            " across ", length(mac_values), " max_ancestor_count value(s)...")
  }

  results <- vector("list", length(min_sizes) * length(mac_values))
  idx <- 0L

  for (mac in mac_values) {
    for (min_size in min_sizes) {
      idx <- idx + 1L

      rollup_result <- tryCatch(
        CLrollup(
          ids                = ids,
          clData             = clData,
          min_group_size     = min_size,
          max_ancestor_count = mac,
          return_mapping     = TRUE,
          verbose            = FALSE
        ),
        error = function(e) NULL
      )

      if (is.null(rollup_result)) {
        results[[idx]] <- list(
          min_group_size     = min_size,
          max_ancestor_count = mac,
          n_groups           = NA_integer_,
          n_rolled           = NA_integer_,
          compression_ratio  = NA_real_,
          deviation          = NA_integer_
        )
        next
      }

      n_groups <- length(rollup_result$rolled_terms)
      n_rolled <- sum(rollup_result$mapping$was_rolled)

      results[[idx]] <- list(
        min_group_size     = min_size,
        max_ancestor_count = mac,
        n_groups           = n_groups,
        n_rolled           = n_rolled,
        compression_ratio  = round(n_groups / length(ids), 3),
        deviation          = abs(n_groups - target_groups)
      )
    }
  }

  # ---- Find best configuration ----
  deviations <- vapply(results, function(r) {
    if (is.null(r) || is.na(r$deviation)) Inf else r$deviation
  }, numeric(1L))

  best_idx <- which.min(deviations)
  best     <- results[[best_idx]]

  if (verbose) {
    mac_str <- if (is.null(best$max_ancestor_count)) "none" else best$max_ancestor_count
    message("\n=== Optimal Resolution for ", target_groups, " target group(s) ===")
    message("  Recommended min_group_size:     ", best$min_group_size)
    message("  Recommended max_ancestor_count: ", mac_str)
    message("  Resulting groups:               ", best$n_groups)
    message("  Terms rolled up:                ", best$n_rolled, " / ", length(ids))
    message("  Compression ratio:              ", best$compression_ratio)

    if (length(results) > 1L) {
      message("\nAlternative configurations:")
      for (i in seq_along(results)) {
        if (i == best_idx) next
        r <- results[[i]]
        if (is.null(r) || is.na(r$n_groups)) next
        mac_s <- if (is.null(r$max_ancestor_count)) "none" else r$max_ancestor_count
        message(sprintf("  min_group_size=%d, max_ancestor_count=%s: %d groups, deviation=%d",
                        r$min_group_size, mac_s, r$n_groups, r$deviation))
      }
    }
  }

  invisible(list(best = best, all_candidates = results))
}
